...
- [ ] 2.2 In `lead_registry.py`, implement `release_lock(harness_home: Path, repo_name: str, lead_name: str) -> None`. Delete the lock file if it exists. Wrap in `try/except OSError` and log warning on failure (never raise — this runs in `finally` blocks which execute even on `KeyboardInterrupt`).
