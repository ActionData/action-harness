...
The `harness lead --repo <path> "prompt"` command SHALL gather repo context and dispatch a Claude Code session with the repo-lead persona. By default, the session SHALL be interactive. The `--non-interactive` flag SHALL trigger the one-shot JSON dispatch.
...
- **WHEN** the user runs `harness lead --repo ./my-repo --non-interactive "What should we work on next?"`
