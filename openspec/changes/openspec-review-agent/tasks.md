...
- [ ] 4.2 In `tests/test_openspec_reviewer.py`: test `dispatch_openspec_review` with mocked subprocess — verify claude CLI invocation args (system prompt, worktree cwd, output format, `--permission-mode bypassPermissions`).
- [ ] 4.3 In `tests/test_openspec_reviewer.py`: test `parse_review_result` — approved result (status=approved, archived=true), findings result (status=findings, archived=false with findings list), malformed JSON (returns error result with "Failed to parse" message).
...
- [ ] 5.1 In `openspec/ROADMAP.md`: add `openspec-review-agent` to the Bootstrap section (already done in this proposal PR).
