...
#### Scenario: Lead detects onboarding gaps
- **WHEN** an interactive lead session starts and `detect_gaps()` reports missing components
- **THEN** the gathered context SHALL include the gap report so the lead agent can offer to run onboarding

#### Scenario: Lead offers onboarding for un-onboarded repo
- **WHEN** the lead agent receives context indicating onboarding gaps exist
- **THEN** the lead SHALL mention the missing components and offer to run `harness onboard` before proceeding with normal suggestions
