...
- [ ] 1.1 In `tests/test_worktree.py`: verify the existing `git_repo` fixture teardown (lines 13-63) actually cleans up all worktrees and temp dirs. If it works correctly, no changes needed. If worktrees leak despite the teardown, fix the teardown logic. Run `ls /tmp/action-harness-*/test-change 2>/dev/null | wc -l` before and after `uv run pytest tests/test_worktree.py` to verify.
