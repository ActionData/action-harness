...
- [x] 4.1 In `_run_review_fix_retry()` in `pipeline.py`, iterate `stages` in reverse order and find the first `WorkerResult` where `success is True`. Use its `session_id` for the fix-retry dispatch. If no successful `WorkerResult` found or `session_id` is None, fall back to fresh dispatch (current behavior).
- [x] 4.2 Add tests: review fix-retry with successful `WorkerResult` having `session_id="sess_abc"` — verify `dispatch_worker` called with `session_id="sess_abc"`. Test without `session_id` — verify fresh dispatch. Test resume failure — verify fallback to fresh dispatch.

## 5. Logging [depends: 3, 4]

- [x] 5.1 Log resume decisions to stderr via `typer.echo(..., err=True)`: "resuming session {session_id} (context {pct}%)" on resume, "context usage {pct}% exceeds threshold, using fresh dispatch" on threshold exceeded, "session resume failed, retrying with fresh dispatch" on resume failure
- [x] 5.2 Add resume-related fields to event logger `worker.dispatched` events: `session_id`, `context_usage_pct`, `resumed: bool`
...
- [x] 6.3 Run tests with `-k session` to verify all session-resume tests pass in isolation
